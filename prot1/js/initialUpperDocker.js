...
function addLogin0(docker){
	...
	docker.buttonSlots[0].menu.bottomButtons.proceedButton.clickFunction = function() {
		if(userPassValid(docker.buttonSlots[0].menu.inputUsername.value, docker.buttonSlots[0].menu.inputPassword.value)) {
			...
			informationNotSavedSuccessfullyZero(docker, addLogin0, "O username introduzido não existe, ou a palavra-passe está errada.");
		}
	}
...
function addFingerprintLogin1(docker) {
	...
	docker.buttonSlots[1].menu.fingerprint.onclick = function() {
		...
		informationSavedSuccessfullyOne(docker);
	}
...
function addUsernameLogin1(docker) {
	...
	docker.buttonSlots[1].menu.bottomButtons.saveButton.clickFunction = function() {	
		...
		if(usernameExists(currentUser.loginUsername)){
			informationNotSavedSuccessfullyOne(docker, addUsernameLogin1, "O username introduzido já existe no sistema. Tente de novo.");
		} else {
			informationSavedSuccessfullyOne(docker);
		}
	}
...
function informationSavedSuccessfullyOne(docker) {
	docker.buttonSlots[1].menu.clean();
	docker.buttonSlots[1].setMenu(new InformationSavedSuccessfully());
	docker.buttonSlots[1].menu.toggleVisibility();
...
function informationNotSavedSuccessfullyOne(docker, f_back, msg) {
	docker.buttonSlots[1].menu.clean();
	docker.buttonSlots[1].setMenu(new InformationNotSavedSuccessfully(msg));
	docker.buttonSlots[1].menu.toggleVisibility();
...
function informationNotSavedSuccessfullyZero(docker, f_back, msg) {
	docker.buttonSlots[0].menu.clean();
	docker.buttonSlots[0].setMenu(new InformationNotSavedSuccessfully(msg));
